...
def validate_api():
    """验证AbuseIPDB API密钥有效性"""
    data = request.get_json()
    api_key = data.get('api_key', '').strip()

    if not api_key:
        return jsonify({'valid': False, 'error': '请输入API密钥'}), 400

    try:
        # 使用一个已知的测试IP来验证API密钥
        test_ip = "8.8.8.8"  # Google DNS，通常是安全的
        ...
        if response.status_code == 200:
            ...
            return jsonify({'valid': False, 'error': f'API验证失败 (状态码: {response.status_code})'}), 400

    except requests.exceptions.Timeout:
        return jsonify({'valid': False, 'error': 'API请求超时'}), 400
    except requests.exceptions.RequestException as e:
        return jsonify({'valid': False, 'error': f'网络请求错误: {str(e)}'}), 400
    except Exception as e:
...
def analyze():
    ...
    if not api_key:
        return jsonify({'error': '请先验证API密钥'}), 400
